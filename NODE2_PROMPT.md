...
### 3.FR — **final recipe 후보** (2026-09-18, 사용자 제안): input+hidden 두 사이트, **점 + ray 6D RoPE**, depth = value 채널 하나(`dpt_chan`, depth 네트워크 없음·layer당 스칼라 gain 1개), **v/o 보류**, 게이트 없음. seed 137, 8-view/30k.
비교 기준(같은 seed, MLP depth): 점+ray 22.903 / 22.809 / 16.943; 점-only 채널 depth 21.995 / 23.006 / 16.741. 판정: 채널 depth가 RE10K·DL3DV에서 MLP 대비 0.1 dB 이내면 채택, 아니면 MLP depth가 본 recipe.
...
### 3.ABL — **최종 recipe 기준 ablation** (2026-09-18 21:50, 사용자 지시). 누적 사다리: (5) NoPE=base → (3) input만 → (4) hidden만 → (2) input+hidden → (1) +v/o. (1)(2)(5)는 완료, (3)(4)는 v/o 없는 단일 사이트. seed 137, `--actckpt`(1·2행과 동일). 런처 `run_queue_0918c.sh`.
| ID | exp | config | 데이터 | 상태 |
|---|---|---|---|---|
| AB-3a | `re10k_dpchan_pdir_in_s137` | `config/dp_chan_pdir_in.yaml` (`foot_in+dpt_chan+pdir`) | RE10K | [QUEUED 21:50] |
| AB-3b | `gobj_dpchan_pdir_in_s137` | 같은 config (`DATA=gobj`) | orbit | [QUEUED] |
| AB-3c | `dl3dvu_dpchan_pdir_in_s137` | 같은 config (`IMG="256 448"`) | DL3DV-u | [QUEUED] |
| AB-4a | `re10k_dpchan_pdir_h_s137` | `config/dp_chan_pdir_h.yaml` (`h_foot+dpt_chan+pdir`) | RE10K | [QUEUED] |
| AB-4b | `gobj_dpchan_pdir_h_s137` | 같은 config (`DATA=gobj`) | orbit | [QUEUED] |
| AB-4c | `dl3dvu_dpchan_pdir_h_s137` | 같은 config (`IMG="256 448"`) | DL3DV-u | [QUEUED] |
...
| NO-4 | `gobj_mat_ext_s137` | 같은 config (`DATA=gobj`) | orbit | [DONE 22.125 (**−0.165 vs base t=−10.3, 29.7% 승**; projection 대비 +0.053 t=3.9) — orbit에서도 평행이동 열만으로 base 아래; 컨테이너 소실 후 처음부터 재학습한 셀] |
...
| NO-6 | `gobj_mat_rot_s137` | 같은 config (`DATA=gobj`) | orbit | [DONE **22.348** (+0.057 vs base t=3.3, 55.5% 승; **+0.222 vs extrinsic t=14.8, 78% 승; +0.276 vs projection t=16.1**) — RE10K와 같은 패턴: 직교 부분만 남기면 base 이상, 평행이동 열을 더하면 −0.2. **NO 연구 종료(16:20), FR 3셀·NO 6셀 전부 완료, GPU 4장 유휴, 큐 비어 있음**] |
...
| ID | exp | config | 데이터 | 상태 |
|---|---|---|---|---|
| D2-1 | `re10k_dpmlp_pcam_s137` | `config/dp_mlp_pcam_both.yaml` (**pcam**: 점 절반 + **카메라 위치 o−p*** 절반, seg 21/hseg 42) | RE10K | [DONE 22.089 (+0.479 vs base; **−0.201 vs 점-only t=−16; −0.814 vs pdir t=−26**) — 카메라 위치 절반은 RE10K에서 해로움] |
| D2-2 | `gobj_dpmlp_pcam_s137` | 같은 config (`DATA=gobj`) | orbit | [DONE 22.563 (+0.272 vs base; **−0.428 vs 점-only t=−28**; −0.246 vs pdir) — orbit에서도 해로움(ray보다 더)] |
| D2-3 | `dl3dvu_dpmlp_pcam_s137` | 같은 config (`IMG="256 448"`) | DL3DV-u | [DONE 16.779 — 점-only(16.888) 대비 −0.11, 점+ray(16.943) 대비 −0.16: 세 데이터 모두 카메라 위치 절반은 손해] |
...
| D2-9 | `dl3dvu_rayrope_s137` | 같은 config (`IMG="256 448"`) | DL3DV-u | [DONE **17.181** (쿼리별, σ₀≈0) — base +0.78; 점+ray+v/o(17.022) 대비 +0.16; DL3DV 최고] |
...
| D2-16 | `gobj_rayropew_s137` | 같은 config (`DATA=gobj`) | orbit | [DONE 22.260 — base −0.03; 점-only 대비 −0.73; pcam 대비 −0.30; 쿼리별 대비 −1.01 — σ 대역이 넓은 채로 남아 점 코드 꺼짐 + 세계좌표 카메라 중심(뷰 ID)만 남음] |
...
| DP-9 | `dl3dvu_dpmem_s137` | `config/dp_mem_both.yaml` | DL3DV-u (`IMG="256 448"`) | [DONE 16.725 (+0.321 vs base t=10.6; +0.076 vs 기존 최고; −0.163 vs dpmlp t=−13)] |
...
| DP-20 | `re10k_dpchan_h_s137` | `config/dp_chan_h.yaml` | RE10K | [DONE 21.929 (+0.319 vs base; −0.066 vs 채널 both; −0.121 vs 채널 input-only)] |
| DP-22 | `gobj_dpchan_h_s137` | `config/dp_chan_h.yaml` (`DATA=gobj`) | orbit | [DONE 22.968 (+0.677 vs base; −0.038 vs 채널 both; +0.055 vs MLP hidden-only)] |
| DP-25 | `re10k_dpmem_in_s137` | `config/dp_mem_in.yaml` | RE10K | [DONE 22.101 (+0.491 vs base; −0.128 vs 메모리 both; +0.182 vs MLP input-only)] |
| DP-26 | `re10k_dpmem_h_s137` | `config/dp_mem_h.yaml` | RE10K | [DONE 22.062 (+0.452 vs base; −0.167 vs 메모리 both; −0.039 vs 메모리 input-only)] |
| DP-34 | `re10k_dpmlp_pdirg_s137` | `config/dp_mlp_pdirg_both.yaml` | RE10K | [DONE 22.817 (+1.207 vs base; **+0.040 vs Plücker t=2.1, 56%** = 근소 우위; −0.086 vs pdir t=−10.5 — θ>20°인 10~20% 장면에서 게이트가 0.6~0.8로 내려가 방향 절반이 약해진 만큼 손해) — **pdirg**: 방향 절반을 장면별 게이트 g=exp(−(θ/45°)²)로 스케일(θ = 입력 뷰 광축 평균 각; RE10K 5°→g≈1, DL3DV 58°→0.2, orbit 95°→0.01). 학습 없음. 다음 빈 GPU 최우선] |
| DP-35 | `gobj_dpmlp_pdirg_s137` | 같은 config (`DATA=gobj`) | orbit | [DONE 22.791 (+0.501 vs base; **−0.200 vs 점-only t=−13**; −0.017 vs pdir = pdir와 동일) — 게이트 0.011로 방향 위상을 거의 0으로 눌렀는데도 pdir 수준: 잔여 위상(최고 rung 0.8 rad) 탓인지 시드 잡음(orbit base sd 0.2)인지 불명 → 하드 게이트(θ<30° 아니면 정확히 0) 셀로 판정] |
...
| DP-21 | `gobj_dpchan_in_s137` | `config/dp_chan_in.yaml` (`DATA=gobj`) | orbit | [DONE 22.762 (+0.471 vs base; −0.244 vs 채널 both; +0.020 vs MLP input-only)] |

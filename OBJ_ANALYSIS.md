...
## 7. 기제 설명 (야간 대수 유도, 2026-09-01 03:00 — 사용자 요청 "왜 그런가"의 답; 실험 L1–L5와 정합)

**핵심 유도**: 열 단위 weight-norm이 고정하는 동작 스케일에서 SwiGLU의 각 hidden 유닛은 입력의 거의
순수한 2차 특징이다(silu(u) ≈ u/2 + u²/4). 따라서 q/k에 얹은 입력 코드는 지배 회수 계수에
⟨Θq, Θk⟩²로 — **짝수(부호 불감) kernel** — 들어가고(홀수 성분은 head 차원분의 1로 억제), h에 얹은
hidden 코드는 같은 계수를 **선형으로**(rung마다 cos Δθ) 곱한다. 이것이 사이트 비대칭의 전부다:
- **입력 사이트가 wrap-내성인 이유**: 위상이 어긋나도 제곱 계수는 양의 바닥으로 퇴화할 뿐 회수를
  뒤집지 못한다. **회전 행렬이 거기서 이기는 이유**: 토큰 특징이 raymap을 통해 카메라 방향과 공변하므로
  R 켤레화가 매칭 내용을 정확히 정렬하고, 제곱이 부분 일치를 이차적으로 억압하는 **softmax 없는 대비
  증강**으로 작동한다. 날카로운 코드가 안전한 것도 짝수 kernel은 음수가 될 수 없기 때문(L5).
- **hidden 사이트가 부호에 민감한 이유**: 선형 곱이라 Δθ가 ±π/2를 넘으면 옳은 value를 **빼 버린다**.
  그래서 대응 토큰에서 차이가 0임이 보장되는 좌표 — ray가 아니라 **ray 위의 3D 점** — 만 허용되고(L1),
  불확실성 봉투(chord의 sinc = 깊이 불확실 구간 위 점 코드의 결맞은 중첩)가 이득이다. 입력이 행렬로
  변별을 이미 공급하면 hidden의 역할은 "선택"이 아니라 "훼손하지 않기"가 되어 보수적 chord가 foot과
  같거나 낫다(L4·L5의 사이트별 서열 역전).
- **carrier가 행렬이어야 하는 이유**: value는 W1에 선형으로 저장되고 모든 readout 채널이 각 value에
  선형이므로 P_i⁻¹ 저장·P_j 출력은 임의의 가역 P에 대해 **대수적으로 정확**하다. 그러나 update가
  Muon(스펙트럼 백색화)·열별 weight-norm·RMSNorm 세 번의 스펙트럼 수술을 거치므로, 크기에 실린 pose는
  지워지고 비등거리 사상은 뒤섞인다 — **직교 행렬만이 최적화기와 교환**한다(L2; 위상 carrier·projective
  carrier의 실패, 순한 진폭 코드의 무효(h_bump)와 camera-lr 실패까지 같은 원리로 설명).
- **세 슬롯이 합성되는 이유**: 셋은 하나의 회수 항의 서로 다른 인자(입력 인자², hidden 인자, 운반되는
  value)를 곱하므로 이득이 대수적으로 분리된 자리에 붙는다(L3). **일관성이 혼합을 이기는 이유**: 회수
  계수는 두 사이트 kernel의 곱이라 두 코드가 같은 쌍을 승인할 때 함께 날카로워지고, 혼합 코드는 서로
  다른 match kernel의 곱이라 한쪽만 승인하는 쌍마다 이중 과세된다.
- **oracle까지의 잔여 격차**: 코드 계열의 실패가 아니라 좌표의 한계 — pose만으로는 query가 자기 ray의
  어느 깊이를 읽을지 알 수 없다. 짝수 입력 kernel은 그 불확실성을 용서하는 슬롯이고, 선형 hidden
  kernel은 그로부터 보호해야 하는 슬롯이다.

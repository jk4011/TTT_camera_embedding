#!/usr/bin/env bash
# DL3DV cell launcher (F50 protocol: 8-view 30k on $DATA_ROOT/dl3dv, eval 140 scenes) with lock + eval, resumable.
...
main() {
  ...
  [ -f $DATA_ROOT/dl3dv/train_index.json ] || { echo "FATAL: $DATA_ROOT/dl3dv missing"; exit 1; }
  ...
  if [ ! -f "$CKPT" ]; then
    ...
    DATA_PATH=$DATA_ROOT/dl3dv/train_index.json STEPS=30000 WARMUP=1500 IMAGE_SIZE="${IMG:-256 256}" bash launch_exp.sh "$GPU" "$EXP" "$CFG" "$SEED"
  fi
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PY eval.py --load "$CKPT" --config "$CFG" --data_path $DATA_ROOT/dl3dv/test_index.json --num_scenes 140 --image_size ${IMG:-256 256} > "outputs/$EXP/eval.log" 2>&1

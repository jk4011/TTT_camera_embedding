#!/bin/bash
# node1 single-GPU chain, v3 (2026-09-24, user decision): once the three NVS dpt_lin cells (absolute depth, linear head) are in,
# RETRAIN the ccv CaPET cell with ABSOLUTE depth from a LINEAR head on x -- unless NVS says absolute depth is clearly worse
...
print('%.4f %s' % (sum(d)/3, ' '.join('%+.3f'%x for x in d)))")
  ...
  if python3 -c "import sys; sys.exit(0 if float('${DELTA%% *}') >= -0.15 else 1)"; then
    say "branch ABS: retraining ccv CaPET with absolute depth from a linear head"
    train capet_lin ccv_capetlin_re
    gen capet_lin ccv_capetlin_re
  else

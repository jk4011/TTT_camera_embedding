#!/bin/bash
# Table 4 (table:ablation_position) with ABSOLUTE depth from a separate LINEAR head (dpt_lin + dpt_abs): 9 cells on ONE GPU.
...
# on node1 as *_dplin_pdir_vo_s137 -- NOT here.
...
main() {
  ...
  export EXTRA_ARGS="--actckpt"   # 3 cells x LPIPS do not fit 178 GB without it (T4-abs dl3dvu OOM, 20:12)
  cd /NHNHOME/WORKSPACE/26msit001_A/jinhyeok/TTT_rope/lact_nvs
  local LOG=outputs/queue_table4_lin.log
  lane() {
    local d=$1 v exp cfg
    for v in both in h; do
      exp=${d}_dplin_pdir_${v}_s137; cfg=config/dp_lin_pdir_${v}.yaml

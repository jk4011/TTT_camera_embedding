...
SRC = "/NHNHOME/WORKSPACE/26msit001_A/jinhyeok/objaverse/renders_v60"
DST = "/tmp/gobj_v60"
...
def convert(job):
    ...
    try:
        ...
        for c in cams:
            if not os.path.isfile(os.path.join(sdir, "views", c["image_name"])):
                raise FileNotFoundError(c["image_name"])
...
def main():
    ...
    test = set(uids)  # v60: every rendered object is an evaluation object
    ...
    with Pool(48) as pool:
        for i, (split, fn, n) in enumerate(pool.imap_unordered(convert, jobs, chunksize=16)):
            if fn is None:
                print("  SKIP bad object", flush=True); continue

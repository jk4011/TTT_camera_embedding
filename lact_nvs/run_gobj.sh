...
[ -f /tmp/gobj/train_index.json ] || { echo "FATAL: /tmp/gobj/train_index.json missing (reshard_gobjaverse.py)"; exit 1; }
...
if [ ! -f "$CKPT" ]; then
  echo "[$EXP] train start $(date)  gpu=$GPU cfg=$CFG seed=$SEED steps=$STEPS depth=${DEPTH_DIR:-none}"
...
CUDA_VISIBLE_DEVICES=$GPU $PY eval.py --load "$CKPT" --config "$CFG" \
  --data_path /tmp/gobj/test_index.json --num_scenes 500 "${DEPTH_EVAL[@]}" \

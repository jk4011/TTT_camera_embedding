"""
From
https://github.com/zhengqili/Crowdsampling-the-Plenoptic-Function/blob/f5216f312cf82d77f8d20454b5eeb3930324630a/models/networks.py#L1478
"""
...
class MsImageDis(nn.Module):
    ...
    def calc_dis_loss(self, input_fake, input_real, gt_mask_0, gt_mask_1):
        ...
        for it, (out0, out1) in enumerate(zip(outs0, outs1)):
            gt_mask_0_x = nn.functional.interpolate(
                gt_mask_0, size=[out0.size(2), out0.size(3)], mode="nearest"
            )
            gt_mask_1_x = nn.functional.interpolate(
                gt_mask_1, size=[out0.size(2), out0.size(3)], mode="nearest"
            )
            ...
            if self.gan_type == "lsgan":
                loss += (
                    torch.sum(((out0 - 0.0) * gt_mask_0_x) ** 2) / num_valid_0
                    + torch.sum(((out1 - 1.0) * gt_mask_1_x) ** 2) / num_valid_1
                )
    ...
    def calc_gen_loss(self, input_fake, gt_mask):
        ...
        for it, (out0) in enumerate(outs0):
            gt_mask_x = nn.functional.interpolate(
                gt_mask, size=[out0.size(2), out0.size(3)], mode="nearest"
            )
...
def upsampleLayer(inplanes, outplanes, upsample="basic", padding_type="zero"):
    ...
    if upsample == "basic":
        upconv = [
            nn.ConvTranspose2d(inplanes, outplanes, kernel_size=4, stride=2, padding=1)
        ]
...
class G_Unet_AdaIN(nn.Module):
    ...
    def forward(self, x):
        input_buffer, style_code = (
            x[:, 0 : -self.style_dim, :, :],
            x[0:1, -self.style_dim :, 0, 0],
        )
...
class UnetBlockAdaIN(nn.Module):
    def __init__(
        ...
        self.upnorm = AdaptiveInstanceNorm(
            outer_nc, style_dim
...
class UnetBlockShallow(nn.Module):
    def __init__(
        ...
        if outermost:
            downconv += [
                nn.Conv2d(input_nc, inner_nc, kernel_size=3, stride=2, padding=p)
            ]
            ...
            innver_conv = [
                nn.Conv2d(input_nc, outer_nc * 2, kernel_size=1, stride=1, padding=0)
            ]
...
class Perceptual(nn.Module):
    def __init__(self):
        ...
        # Download and place to
        # "~/.cache/torch/hub/checkpoints/imagenet-vgg-verydeep-19.mat"
        url = "https://www.vlfeat.org/matconvnet/models/imagenet-vgg-verydeep-19.mat"
        model_root = os.path.join(
            os.path.expanduser("~"), ".cache/torch/hub/checkpoints/"
        )
        model_path = os.path.join(model_root, "imagenet-vgg-verydeep-19.mat")
        ...
        with torch.no_grad():
            ...
            vgg_rawnet = sio.loadmat(model_path)
    ...
    def forward(self, pred_img, real_img):
        ...
        bb = torch.from_numpy(aa).float().permute(0, 3, 1, 2).to(pred_img.device)

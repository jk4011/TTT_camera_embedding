...
> 13:00 추가: hidden ladder ×2는 **chord hidden(rot_hshell)에선 −0.238로 해로움** → h2x 이득은 vi의 foot hidden 코드 전용(봉투 없는 점 코드만 해상도 이득; sinc 봉투는 과협착). 스펙트럼 정합 주장은 철회, 슬롯 대수(제곱/선형/직교)만 유지.
> 11:50 추가: ×4 반증 셀이 **23.055 = +1.074**(h2x 대비 +0.059, t=6.1) — "×2 최적" 예측은 반증, vi에선 ×4까지 단조
> 증가(증분 체감 +0.165→+0.059). ×8 탐침 진행 예정. 즉 hidden 사다리 스케일은 데이터별 손잡이(vi↑, orbit↓).
> 11:20 정정: orbit 쌍둥이에서 h2x가 **−0.171**(vs orbit iso) → hidden 사다리 스케일은 데이터 의존. **교차 데이터셋
> 강건 레시피 = `foot_all_iso`**(vi 3-seed 평균 +0.825, orbit +0.815, RE10K 검증 중), `foot_all_iso_h2x`는 vi 전용
> 최고(+1.016/+1.074). 후속 아이디어: 사다리를 상수 대신 장면별 foot 좌표 분산에 맞춰 스케일(정규화 인지).
...
- 시드 재현(규약: 셀 시드평균 vs base 시드평균, ±시드 std; vi 3-seed 표 완성 — 대장 F81 부록 5): base 21.984±0.099; iso_h2x 23.002±0.044 (Δ +1.018); foot_all 22.535±0.176 (Δ +0.551, 가장 불안정); **`foot_all_iso` 22.809±0.029 (Δ +0.825)**; rot_hshell 22.597±0.087 (Δ +0.613; s211 +0.461로 폭 넓음); `foot_all_iso`(iso만) 3-seed +0.851 / +0.890 / +0.734(각 시드 base와 짝지음,
  t≥40). rot_hshell 2-seed +0.716/+0.661. **`foot_all_iso_h2x` 3-seed 23.002±0.044 (Δ +1.018)** — vi에서 3-seed 모두 +0.96~+1.07. orbit 쌍둥이는 §1 상단 정정 참조.
...
| orbit 91° 499 | +0.642 | +0.564 (iso **+0.815**, t=35; iso×h2 +0.644) | iso는 전이(orbit 1위), **h2x는 orbit에 비전이(−0.171)** |
| RE10K narrow 256 | **+0.266** (t=12) | **+0.481** (t=15) | 좁은 베이스라인도 이득, foot_all 우위 |
| DL3DV 부분호 140 | −0.001 | −0.015 | **정확히 중립** — 큰 반경 walk-around 궤도(원/반원, 도보 지름 30–45초, 수평 180–360°, 머리/허리 2회 주행)인데 로더는 장면의 **연속 부분구간만** 서빙(평가=중앙 정렬 min(num_frames,128), `eval.py --window` 기본 128 / 학습=길이 45–192 무작위 연속구간, `train.py --window` 기본 192) + unbounded 야외라 **단일 중심 부재** → p\*(광축 최소자승 교점)가 약하게만 결정되어 좌표 degenerate, 학습형 gain이 코드 무음화. 무해·무이득. F50 측정: 서빙된 12뷰의 forward axis **쌍별 각도 median 34.5° / mean 49.0°**. ※2026-09-01 정정: 이전 표기 "전진 보행에선 p\*가 비정칙"은 오류 — DL3DV 공식 촬영은 원/반원이고, 전진 보행이면 forward axis가 거의 평행(≈0°)이라 34.5° 측정과 모순. 수치 불변, 메커니즘 설명만 정정 |

# tttLRM 노드 작업 지시 (node1이 작성, 2026-09-24 21:20 KST; 21:45 갱신: 1-GPU 노드가 먼저 할당되어 거기서 돌린다)

**1-GPU 노드면**: 1-GPU 설정(grad_accum 2, 유효 배치 8 동일)으로 자동 전환, 15,000스텝 × 5.8초 ≈ 24시간 + 평가 0.5시간.
**2-GPU 노드로 옮길 때**: 반드시 1-GPU 쪽 학습을 먼저 멈춘 뒤 같은 명령을 실행한다(둘이 같은 체크포인트 폴더에 쓴다). `latest.pt`에서 이어진다.

...
TRAIN_BLOCKS = os.path.join(DNA_DIR, "hg38_train_blocks_4096.npy")
VAL_CACHE = os.path.join(LACT_LLM_DIR, "val_cache_hg38_4096.pt")
...
def ensure_train_blocks():
    ...
    if os.path.exists(TRAIN_BLOCKS):
        return TRAIN_BLOCKS
    ...
    tmp = TRAIN_BLOCKS + ".tmp.npy"
    np.save(tmp, allb)
    os.replace(tmp, TRAIN_BLOCKS)
    print(f"[dna] train blocks: {allb.shape[0]:,} x {SEQ_LEN} "
          f"({allb.nbytes / 1e9:.1f} GB) -> {TRAIN_BLOCKS} in {time.time() - t0:.0f}s",
          flush=True)
    return TRAIN_BLOCKS


def get_or_build_dna_val_set(n_blocks, cache_path):
    ...
    if os.path.exists(cache_path):
        ...
        print(f"[dna] WARNING: cached val {cache_path} shape {tuple(v.shape)} "
              f"insufficient for {n_blocks} blocks; rebuilding.", flush=True)
    blk = None
...
def main():
    ...
    ensure_train_blocks()
    get_or_build_dna_val_set(488, VAL_CACHE)
    print("[dna] preprocessing complete.", flush=True)

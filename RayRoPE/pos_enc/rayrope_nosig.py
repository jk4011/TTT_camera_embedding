...
class RayRoPE_NoSig_DotProductAttention(torch.nn.Module):
    """ 
    Self-Attention with RayRoPE without depth uncertainty (sigma) prediction.
    See rayrope_release.py for options
    """
    ...
    def forward(
        ...
        predicted_d: Optional[torch.Tensor] = None,  # (batch, seqlen, 1)
    ...
    def _prepare_apply_fns(
        ...
        for cam_idx in range(num_cameras):
            ...
            if self.pd_type == '3d':
                ...
                pd_d = pd_d.flatten(start_dim=-2, end_dim=-1)
            ...
            cos_KV, sin_KV = _prepare_rope_coeff(positions_KV, self.num_rope_freqs, self.freq_base, batch, num_cameras, num_patches) # (batch, num_cameras, num_patches, num_freqs, num_coord)
        ...
        cos_Q, sin_Q = _prepare_rope_coeff(positions_Q, self.num_rope_freqs, self.freq_base, batch, num_cameras, num_patches)
...
def _transform_to_query_frame(
    points_world: torch.Tensor,  # (batch, num_cameras, num_patches, num_rays_per_patch, 4)
    ...
    if transform_type == '3d':
        points_cam = torch.einsum("bij,...bcprj->...bcpri", w2c, points_world) # (batch, num_cameras, num_patches, num_rays_per_patch, 4)
        if norm_by == 'w':
...
def _get_point_coords(
        ...
        depths: torch.Tensor = None,  # (batch, num_cameras, num_patches, num_rays_per_patch, 1)
    ) -> torch.Tensor:
    ...
    if depths is not None:
        ...
        disparity = 1 / depths
        # Only handle single depth tensor (no uncertainty)
        assert depths.ndim == 5, f"Expected depths.ndim == 5, got {depths.ndim}"
...
def _prepare_depths(
    predicted_d: Optional[torch.Tensor],  # (batch, seqlen, 1)
    ...
    """Prepare depths for predict_d mode (no uncertainty)."""
    # predicted_d is (batch, seqlen, 1) - just log depth, no sigma
    predicted_d = predicted_d.reshape(batch, num_cameras, num_patches, 1, 1)
    predicted_logd = predicted_d[..., 0:1]

    # Convert log depth to depth
    depths = torch.exp(torch.clamp(predicted_logd, max=MAX_LOG_DEPTH))
    depths = depths.expand(-1, -1, -1, num_rays_per_patch, -1)  # (batch, num_cameras, num_patches, num_rays_per_patch, 1)
    ...
    if 'known' in depth_type:
        ...
        # (batch, num_cameras, num_patches, num_rays_per_patch, 1)
...
def _prepare_rope_coeff(
    ...
    """Prepare normal rope coefficients (no expected value computation)."""
    coord_dim = 0
    ...
    for pos_name, pos in positions.items():
        ...
        # Normal rope coefficients - just cos/sin without expected value computation
